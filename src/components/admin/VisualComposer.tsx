...
import { VisualComposerClient, type MediaOption, type PageSummary } from '@/components/admin/VisualComposerClient'
...
type ComposerPayload = {
  find: (options: {
    ...
    pagination?: boolean
    sort?: string
  }) => Promise<{ docs: Array<MediaOption | PageSummary> }>
}
...
async function getMediaOptions(payload: ComposerPayload) {
  try {
    const result = await payload.find({
      collection: 'media',
      depth: 0,
    ...
    return result.docs as MediaOption[]
  } catch {
    return []
  }
}
...
export async function VisualComposer(props: AdminViewServerProps) {
  ...
  const mediaOptions = await getMediaOptions(payload)
